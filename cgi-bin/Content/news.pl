...
sub show
{
    ...
    my %parameter = (
                  ...
                  body      => translate('body'),
                  class     => 'max',
                  attach    => $m_nRight >= $m_hrSettings->{uploads}{right} ? $m_hrSettings->{uploads}{enabled} : 0,
    ...
    if (not defined $sbm or ($sbm ne translate('preview'))) {
        if (defined param('message') && defined param('headline') && defined param('thread') && defined param('catlist')) {
            ...
            if (defined $headline && defined $message && defined $thread && $m_nRight >= $m_hrSettings->{news}{right}) {
                ...
                if   ($m_oDatabase->addMessage(\%message)) {$m_sContent .= '<div align="center">Nachricht wurde erstellt.<br/></div>';}
                else                                       {$m_sContent .= '<div align="center">' . translate('floodtext') . '<br/></div>';}
            }
        }
        &show();
    } else {
...
sub editNews
{
    ...
    if (not defined param('submit') or (param('submit') ne translate('preview'))) {
        ...
        my %parameter = (
                  ...
                  body      => $data[1],
                  class     => 'max',
                  attach    => $m_nRight >= $m_hrSettings->{uploads}{right} ? $m_hrSettings->{uploads}{enabled} : '',
                  maxlength => $m_hrSettings->{news}{maxlength},
                  path      => "$m_hrSettings->{cgi}{bin}/templates",
                  reply     => $id,
                  server    => $m_hrSettings->{cgi}{serverName},
...
sub replyNews
{
    ...
    $th = ($th =~ /^(\w+)$/) ? $1 : 'trash';
    my $attachment;
    if ($m_nRight >= $m_hrSettings->{uploads}{right}) {$attachment = $m_hrSettings->{uploads}{enabled};}
    else {
...
{
    my $id = shift;
    if (defined param('reply') && param('reply') =~ /(\d+)/) {$id = $1 unless (defined $id);}
    ...
    if ($ref->{id} == $id) {
        ...
        my $menu = "";
        my $answerlink =
          $m_hrSettings->{cgi}{mod_rewrite} ? "/replynews-$ref->{id}.html" : "$ENV{SCRIPT_NAME}?action=reply&amp;reply=$ref->{id}&amp;thread=news";
        my %reply = (
                     title    => translate('reply'),
        ...
        $m_sContent .= qq(<tr><td><a href="/downloads/$ref->{attach}">$ref->{attach}</a></td></tr>)
...
sub readcats
{
    ...
    for (my $i = 0; $i <= $#cats; $i++) {
        ...
        $list .=
          $sel{$catname} ? qq(<option value="$catname"  selected="selected">$catname</option>) : qq(<option value="$catname">$catname</option>);
    }
    $list .= '</select>';
    return $list;
}
                   ...
                   div(
                       ...
                       (
                        ...
                        : ''
                         ...
                         . ($length > 5 ? translate('news_pro_page') . '&#160;|&#160;' : '')
                         . (
                            $length > 5
                            ? a(
                                {
...
qq/select title,body,date,id,user,attach,format from $th where `right` <= $m_nRight $answers  order by date desc LIMIT $m_nStart,$lpp /;
        ...
        while (my @data = $sth->fetchrow_array()) {
            ...
            my $attach      = $data[5];
            my $format      = $data[6];
            my $replylink = $m_hrSettings->{cgi}{mod_rewrite} ? "/news$id.html" : "$ENV{SCRIPT_NAME}?action=showthread&amp;reply=$id&amp;thread=$th";
            ...
            if ($th ne 'replies') {
                my $answerlink =
                  $m_hrSettings->{cgi}{mod_rewrite} ? "/reply$th-$id.html" : "$ENV{SCRIPT_NAME}?action=reply&amp;reply=$id&amp;thread=$th";
                my %reply = (
                             title    => translate('reply'),
                             descr    => translate('reply'),
                             src      => 'reply.png',
            ...
            my $permalink = $m_hrSettings->{cgi}{mod_rewrite} ? "/news$id.html" : "$ENV{SCRIPT_NAME}?action=showthread&amp;thread=$th&amp;reply=$id";
