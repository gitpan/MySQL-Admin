...
foreach my $key (sort keys %{$m_hrLng}) {
    push @l, $key;
    $m_sContent .= a({href => "$ENV{SCRIPT_NAME}?action=translate&lang=$key"}, $key) . '&#160;|&#160;';
}
...
sub on_valid_form
{
    ...
    my $window = new HTML::Window(\%parameter);
    $m_sContent .= $window->windowHeader();
    $m_sContent .= '<br/><div align="center" style="width:75%;"><b>' . translate('Done') . qq(&#160;<a href="$rs">) . translate('next') . '</a></b><br/><div align="left">';
    my @entrys = param();
    for (my $i = 0; $i <= $#entrys; $i++) {
        my $rkey = lc $entrys[$i];
        delete $m_hrLng->{$lg}{$entrys[$i]};
        $m_sContent .= "$rkey: " . param($entrys[$i]) . '<br/>' unless (($rkey eq 'action') or ($rkey eq 'checkFormsddfsds'));
        $m_hrLng->{$lg}{$rkey} = param($entrys[$i]) unless ($rkey eq 'action' or ($rkey eq 'checkFormsddfsds'));
    }
