...
sub ExecSql {
    ...
    $RIBBONCONTENT .=
        '<br/><div class="sqlBox" style="width:100%;overflow:auto;">'
        if $showSql;
    my $id2 = 0;
...
sub SaveRights {
    ...
    if ( defined $session ) {
        ...
        push @BOOL, "CREATE TEMPORARY TABLES"
            if param(
            'marksBox' . $m_hrParams->{BOOL}{CREATETEMPORARYTABLES} ) eq 'on';
        ...
        push @REVEOKE, "CREATE TEMPORARY TABLES"
            if param(
            'marksBox' . $m_hrParams->{BOOL}{CREATETEMPORARYTABLES} ) ne 'on';
