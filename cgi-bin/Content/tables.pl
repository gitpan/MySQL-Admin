...
sub ShowNewTable {
    ...
    for ( my $j = 0; $j < $count; $j++ ) {
        ...
        $vars{rows}{$m_hUniqueField} = { Field   => $m_hUniqueField,
                                         Type    => $m_hUniqueType,
...
sub ShowDumpTable {
    ...
    $m_sContent .=
        qq(<textarea style="width:100%;height:800px;overflow:auto;">);
    DumpTable($tbl);
...
sub ExecSql {
    ...
    $RIBBONCONTENT .=
        '<br/><br/><div class="sqlBox" style="width:100%;overflow:auto;">'
        if $showSql;
    my $id2 = 0;
    my $ret = 1;
...
sub ShowTable {
    ...
    if ( $m_oDatabase->tableExists($tbl) ) {
        ...
        for ( my $i = 0; $i <= $#a; $i++ ) {
            ...
            $m_sContent .=
                qq|<td width="20"><input type="checkbox" name="markBox$i" class="markBox" value="$tbl/$pkey" /></td>|;
            for ( my $j = 0; $j <= $rows; $j++ ) {
                my $headline = $a[$i]->{ $caption[$j]->{'Field'} };
                $m_sContent .= '<td class="values">'
                    . substr( $headline, 0,
                              int( 120/ ( $rows > 0 ? $rows : 1 ) ) )
                    . '</td>';
            }
            $m_sContent .=
    ...
    for ( my $i = 0; $i <= $#params; $i++ ) {
        if ( $params[$i] =~ /markBox\d?/ ) {
            ...
            if ( $#p_key > 0 ) {
                for ( my $j = 0; $j < $#p_key; $j++ ) {
                    $eid .=
                          $m_dbh->quote_identifier( $p_key[$j] ) . ' = '
                        . $m_oDatabase->quote( $prims[$j] ) . ' && ';
                }
                $eid .=
                      $m_dbh->quote_identifier( $p_key[$#p_key] ) . ' = '
                    . $m_oDatabase->quote( $prims[$#p_key] );
            } else {
                $eid .=
                      $m_dbh->quote_identifier( $p_key[0] ) . ' = '
                    . $m_oDatabase->quote( $prims[0] );
            }
        SWITCH: {
                if ( $a eq "delete" ) {
...
sub EditEntry {
    ...
    if ( $m_oDatabase->tableExists($tbl) ) {
        ...
        if ( $#p_key > 0 ) {
            for ( my $j = 0; $j < $#p_key; $j++ ) {
                $eid .= "$p_key[$j] = "
                    . $m_oDatabase->quote( param( $p_key[$j] ) ) . ' && ';
            }
            $eid .= "$p_key[$#p_key] = "
...
sub EditVertMenu {
    ...
    my @id = $m_oDatabase->fetch_array(
                                 "select id from `navigation` where action=?",
                                 $name );
...
sub EditTopMenu {
    ...
    my $name = defined param('name') ? param('name') : $m_sAction;
    my @id = $m_oDatabase->fetch_array(
                              "select id from `topnavigation` where action=?",
                              $name );
        ...
        while ( $i < $#params ) {
            ...
            if ( $params[$i] =~ /tbl.*/ ) {
                ...
                if ( $#p_key > 0 ) {
                    ...
                    $eid .= "$p_key[0] = " . $m_oDatabase->quote($pa)
...
#                       </form></td></table></td>|;
        ...
        for ( my $j = 0; $j <= $#caption; $j++ ) {
            ...
            $m_sContent .= qq|
              ...
              <td>|
                . $m_oDatabase->GetNull( $caption[$j]->{'Null'},
                                         $m_hUniqueNull )
                . qq|</td>
            ...
            $vars{rows}{$field} = { Field     => $m_hUniqueField,
                                    Type      => $m_hUniqueType,
                                    Length    => $m_hUniqueLength,
                                    Null      => $m_hUniqueNull,
                                    Default   => $m_hUniqueDefault,
        ...
        my $qstringCol = createSession({ user      => $m_sUser,
        ...
        my $atrrs =
            $m_oDatabase->GetAttrs( $tbl, "none", $m_hUniqueColAttrs );
...
sub ShowEditIndex {
    ...
    my $hashref =
        $m_oDatabase->fetch_hashref(
                                 "SHOW INDEX FROM $tbl2 where `Key_name` = ?",
                                 $iname );
    $RIBBONCONTENT .= qq|
              <div align="center">
              <form action="$ENV{SCRIPT_NAME}" method="post" enctype="multipart/form-data">
    ...
    if ( param('editIndexOlp145656') ) {
        ...
        for ( my $i = 0; $i < $#index; ++$i ) {
            ...
            my $uSize = Unique();
            my $columns =
                $m_oDatabase->GetColumns( $tbl, $uName,
                                          $index[$i]->{Column_name} );
            $RIBBONCONTENT .=
    ...
    my $qstring = createSession( { user   => $m_sUser,
                                   action => 'SaveNewIndex',
                                   table  => $tbl,
...
sub SaveNewIndex {
    ...
    if ( defined $tbl and defined $session ) {
        ...
        for ( my $i = 0; $i <= $#{ $m_hrParams->{fields} }; $i++ ) {
            ...
            my $m_nSize =
                param( $m_hrParams->{fields}[$i]{size} ) =~ /(\d+)/ ? $1 : 0;
            $sql .= qq|$field|;
...
sub SaveEditTable {
    ...
    if ( defined $tbl and defined $session ) {
        ...
        my $key = join( ' , ', @prims );
        $sql .=
            "ALTER TABLE $tbl2 DROP PRIMARY KEY, ADD constraint PRIMARY KEY ($key);$/";
        ExecSql($sql);
        EditTable($tbl);
    } else {
        ShowTables();
...
sub ShowDbHeader {
    ...
    $m_sContent .= a( {  class => $current eq "NewEntry"
                         ? 'currentLink'
                         : 'link',
...
<select onSubmit="return false;" id="tablelist" name="tablelist" size="10" onkeypress="var keyCode = event.keyCode ? event.keyCode :event.charCode ? event.charCode :event.which;if (keyCode != 13) return;var e = document.getElementById('sqlEdit');e.value +=this.options[this.options.selectedIndex].value;return false;" style="width:150px;height:100px;">|;
    ...
    for ( my $i = 0; $i <= $#tables; $i++ ) {
        ...
        $list .=
            qq(<option value="$name" onclick="DisplayTable('$tables[$i]');" ondblclick="var e = document.getElementById('sqlEdit');e.value +='$name';">$tables[$i]</option>);
    }
    $list .= '</select>';
    ...
    for ( my $i = 0; $i <= $#tables; $i++ ) {
        ...
        $list .=
            qq|<select onSubmit="return false;" id="$tables[$i]" size="10" onkeypress="var keyCode = event.keyCode ? event.keyCode :event.charCode ? event.charCode :event.which;if (keyCode != 13) return;var e = document.getElementById('sqlEdit');e.value +=this.options[this.options.selectedIndex].value;return false;" style="display:none;width:150px;height:100px;">|;
        for ( my $i = 0; $i <= $#tables2; $i++ ) {
            my $name = $m_dbh->quote_identifier( $tables2[$i]->{'Field'} );
            $list .=
                qq(<option value="$name" ondblclick="var e = document.getElementById('sqlEdit');e.value +='$name';">$tables2[$i]->{'Field'}</option>);
        }
        $list .= '</select></div>';
    }
    ...
    for ( my $i = 0; $i <= $#tables; $i++ ) {
        $list .=
            qq(<option value="$tables[$i]" onkeydown="return false;" ondblclick="var e = document.getElementById('sqlEdit');e.value +='$tables[$i]';">$tables[$i]</option>);
    }
    $list .= '</select>';
...
sub initRights {
    ...
    my $hr =
        $m_oDatabase->fetch_hashref(
                        "SELECT * FROM mysql.user where USER = ? && Host = ?",
                        $p_sUser, $p_sHost );
    foreach ( keys %{$hr} ) {
        if ( $_ =~ /(.*)_priv$/ ) {
            my $key = lc($1);
...
sub SaveRights {
    ...
    if ( defined $session ) {
        ...
        my @BOOL;
        push @BOOL, "UPDATE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{UPDATE} ) eq 'on';
        push @BOOL, "DELETE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{DELETE} ) eq 'on';
        push @BOOL, "CREATE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATE} ) eq 'on';
        push @BOOL, "DROP"
            if param( 'marksBox' . $m_hrParams->{BOOL}{DROP} ) eq 'on';
        push @BOOL, "RELOAD"
            if param( 'marksBox' . $m_hrParams->{BOOL}{RELOAD} ) eq 'on';
        push @BOOL, "SHUTDOWN"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHUTDOWN} ) eq 'on';
        push @BOOL, "PROCESS"
            if param( 'marksBox' . $m_hrParams->{BOOL}{PROCESS} ) eq 'on';
        push @BOOL, "FILE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{FILE} ) eq 'on';
        push @BOOL, "REFERENCES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REFERENCES} ) eq 'on';
        push @BOOL, "INDEX"
            if param( 'marksBox' . $m_hrParams->{BOOL}{INDEX} ) eq 'on';
        push @BOOL, "ALTER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{ALTER} ) eq 'on';
        push @BOOL, "SHOW DATABASES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHOWDATABASES} ) eq
                'on';
        push @BOOL, "SUPER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SUPER} ) eq 'on';
        push @BOOL, "CREATE TEMPORARY TABLES"
            if param('marksBox' . $m_hrParams->{BOOL}{CREATETEMPORARYTABLES} )
                eq 'on';
        push @BOOL, "LOCK TABLES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{LOCKTABLES} ) eq 'on';
        push @BOOL, "REPLICATION CLIENT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REPLICATIONCLIENT} ) eq
                'on';
        push @BOOL, "CREATE VIEW"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEVIEW} ) eq 'on';
        push @BOOL, "SHOW VIEW"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHOWVIEW} ) eq 'on';
        push @BOOL, "CREATE ROUTINE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEROUTINE} ) eq
                'on';
        push @BOOL, "ALTER ROUTINE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{ALTERROUTINE} ) eq
                'on';
        push @BOOL, "CREATE USER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEUSER} ) eq 'on';
        push @BOOL, "REPLICATION SLAVE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REPLICATIONSLAVE} ) eq
                'on';
        push @BOOL, "INSERT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{INSERT} ) eq 'on';
        push @BOOL, "SELECT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SELECT} ) eq 'on';
        push @BOOL, "EXECUTE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{EXECUTE} ) eq 'on';
        push @BOOL, "UPDATES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{UPDATES} ) eq 'on';
        push @BOOL, "GRANT OPTION"
            if param( 'marksBox' . $m_hrParams->{BOOL}{GRANT} ) eq 'on';
        ...
        if ( $#BOOL > 0 ) {
            ...
            $sql .= 'MAX_QUERIES_PER_HOUR '
                . param( $m_hrParams->{NUMBER}{MAX_QUERIES_PER_HOUR} );
            $sql .= ' MAX_CONNECTIONS_PER_HOUR '
                . param( $m_hrParams->{NUMBER}{MAX_CONNECTIONS_PER_HOUR} );
            $sql .= ' MAX_UPDATES_PER_HOUR '
                . param( $m_hrParams->{NUMBER}{MAX_UPDATES_PER_HOUR} );
            $sql .= ' MAX_USER_CONNECTIONS '
                . param( $m_hrParams->{NUMBER}{MAX_USER_CONNECTIONS} );
            $sql .= ';';
            ExecSql($sql);
        }
        $sql = 'REVOKE ';
        my @REVEOKE;
        push @REVEOKE, "UPDATE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{UPDATE} ) ne 'on';
        push @REVEOKE, "DELETE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{DELETE} ) ne 'on';
        push @REVEOKE, "CREATE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATE} ) ne 'on';
        push @REVEOKE, "DROP"
            if param( 'marksBox' . $m_hrParams->{BOOL}{DROP} ) ne 'on';
        push @REVEOKE, "RELOAD"
            if param( 'marksBox' . $m_hrParams->{BOOL}{RELOAD} ) ne 'on';
        push @REVEOKE, "SHUTDOWN"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHUTDOWN} ) ne 'on';
        push @REVEOKE, "PROCESS"
            if param( 'marksBox' . $m_hrParams->{BOOL}{PROCESS} ) ne 'on';
        push @REVEOKE, "FILE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{FILE} ) ne 'on';
        push @REVEOKE, "REFERENCES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REFERENCES} ) ne 'on';
        push @REVEOKE, "INDEX"
            if param( 'marksBox' . $m_hrParams->{BOOL}{INDEX} ) ne 'on';
        push @REVEOKE, "ALTER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{ALTER} ) ne 'on';
        push @REVEOKE, "SHOW DATABASES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHOWDATABASES} ) ne
                'on';
        push @REVEOKE, "SUPER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SUPER} ) ne 'on';
        push @REVEOKE, "CREATE TEMPORARY TABLES"
            if param('marksBox' . $m_hrParams->{BOOL}{CREATETEMPORARYTABLES} )
                ne 'on';
        push @REVEOKE, "LOCK TABLES"
            if param( 'marksBox' . $m_hrParams->{BOOL}{LOCKTABLES} ) ne 'on';
        push @REVEOKE, "REPLICATION CLIENT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REPLICATIONCLIENT} ) ne
                'on';
        push @REVEOKE, "CREATE VIEW"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEVIEW} ) ne 'on';
        push @REVEOKE, "SHOW VIEW"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SHOWVIEW} ) ne 'on';
        push @REVEOKE, "CREATE ROUTINE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEROUTINE} ) ne
                'on';
        push @REVEOKE, "ALTER ROUTINE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{ALTERROUTINE} ) ne
                'on';
        push @REVEOKE, "CREATE USER"
            if param( 'marksBox' . $m_hrParams->{BOOL}{CREATEUSER} ) ne 'on';
        push @REVEOKE, "REPLICATION SLAVE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{REPLICATIONSLAVE} ) ne
                'on';
        push @REVEOKE, "INSERT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{INSERT} ) ne 'on';
        push @REVEOKE, "SELECT"
            if param( 'marksBox' . $m_hrParams->{BOOL}{SELECT} ) ne 'on';
        push @REVEOKE, "EXECUTE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{EXECUTE} ) ne 'on';
        push @REVEOKE, "UPDATE"
            if param( 'marksBox' . $m_hrParams->{BOOL}{UPDATE} ) ne 'on';
        push @REVEOKE, "GRANT OPTION"
            if param( 'marksBox' . $m_hrParams->{BOOL}{GRANT} ) ne 'on';
    ...
    ShowRights( param( $m_hrParams->{DBUSER} ),
                param( $m_hrParams->{HOST} ) );
}
