...
show_form(
    ...
    -FIELDS   => [
        ...
        },
        {
         -LABEL     => 'sidebarLeft',
         -TYPE      => 'scrolling_list',
         '-values'  => [($m_hrSettings->{sidebar}{left}) ? ('Enabled', 'Disabled') : ('Disabled', 'Enabled')],
         -size      => 1,
         -multiples => 0,
        ...
        },
        {
         -LABEL     => 'sidebarRight',
         -TYPE      => 'scrolling_list',
         '-values'  => [($m_hrSettings->{sidebar}{right}) ? ('Enabled', 'Disabled') : ('Disabled', 'Enabled')],
         -size      => 1,
         -multiples => 0,
         -VALIDATE  => \&enabledDisabled,
        },
        ...
        },
        {
         -LABEL     => 'activates',
         -TYPE      => 'scrolling_list',
         '-values'  => [($m_hrSettings->{uploads}{enabled}) ? ('Enabled', 'Disabled') : ('Disabled', 'Enabled')],
         -size      => 1,
         -multiples => 0,
         -VALIDATE  => \&enabledDisabled,
        },
...
sub on_valid_form
{
    ...
    #boxes
    for (my $i = 0; $i <= $#a; $i++) {
        $m_oDatabase->void("update box set `position`= '" . param($a[$i]->{name}) . "'  where name= '" . $a[$i]->{name} . "'");
    }
    MySQL::Admin::Settings::saveSettings("$m_hrSettings->{cgi}{bin}/config/settings.pl");
    $m_sStyle = $p1;
    my $htm = GetHtml();
    my $rs = ($m_hrSettings->{cgi}{mod_rewrite}) ? '/settings.html' : "$ENV{SCRIPT_NAME}?action=settings";
    my %parameter = (
                     path   => $m_hrSettings->{cgi}{bin} . '/templates',
                     style  => $m_sStyle,
    ...
    my $window = new HTML::Window(\%parameter);
    $m_sContent .= '<div align="center"><b>Done</b><br/>' . qq(<a href="$rs">) . translate('next') . '</a>';
    my @entrys = param();
    $m_sContent .= $window->windowHeader();
    for (my $i = 0; $i <= $#entrys; $i++) {$m_sContent .= "$entrys[$i]: " . param($entrys[$i]) . '<br/>';}
    $m_sContent .= $window->windowFooter();
