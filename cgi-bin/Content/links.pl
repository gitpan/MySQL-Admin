...
sub ShowBookmarks
{
    ...
    *t = \@{$HTML::Menu::TreeView::TreeView[0]};
    ...
    _showBookmarksNavi();
    $m_sContent .= qq(<table align="center"  border="0" cellpadding="0" cellspacing="0"  width="100%" summary="linkLayout"><tr><td valign="top">);
    $m_sContent .= Tree(\@t);
...
sub _showBookmarksNavi
{
      ...
      . a(
          {
           ...
           href  => "$ENV{SCRIPT_NAME}?action=links&sort=$ss&folderfirst=$folderfirst",
    ...
    $m_sContent .= a(
                     {
                      ...
                      href  => "$ENV{SCRIPT_NAME}?action=links&sort=$sortstate&folderfirst=$ff",
    ...
    $m_sContent .= '&#160;|&#160;'
      . a(
          {
           class   => 'link',
           href    => qq|javascript:window.open(' $m_hrSettings->{cgi}{serverName}$ENV{SCRIPT_NAME}?dump=links&action=newTreeviewEntry&addBookMark='+escape(location.href)+'&title='+escape(document.title),'Bookmark');void(0)|,
...
sub ExportOperaBookmarks
{
    ...
    *t = \@{$HTML::Menu::TreeView::TreeView[0]};
    ...
    $m_sContent .= br() . $window->windowHeader();
    _showBookmarksNavi();
    $m_sContent .= qq(<table align="center"  border="0" cellpadding="0" cellspacing="0"  width="100%" summary="linkLayout"><tr><td align="center" valign="top">);
    $m_sContent .= qq(<textarea style="width:98%;height:800px;">\nOpera Hotlist version 2.0\nOptions: encoding = utf-8, version=3\n);
    &_rec(\@t);
