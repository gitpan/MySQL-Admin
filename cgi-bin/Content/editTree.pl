...
sub moveEntry
{
    ...
    for (my $i = 0; $i <= @$t; $i++) {
        ...
        if (@$t[$i]) {
            ...
            if (defined @{@$t[$i]->{subtree}}) {moveEntry(\@{@$t[$i]->{subtree}}, $find);}
        }
    }
}
    ...
    for (my $i = 0; $i < @$t; $i++) {
        if (@$t[$i]->{rid} == $find) {
            ...
            foreach my $key (sort(keys %params)) {
                $node->{$key} = $params{$key}
                  if ($params{$key} && $key ne 'action' && $key ne 'folder' && $key ne 'subtree' && $key ne 'class' && $key ne 'dump');
                $node->{$key} = ($m_hrSettings->{cgi}{mod_rewrite} ? "/$1.html" : "$ENV{SCRIPT_NAME}?action=$1")
                  if ($key eq 'href' && $params{$key} =~ /^action:\/\/(.*)$/);
            }
            if (param('folder')) {$node->{'subtree'} = [{text => 'Empty Folder',}];}
            if (param('addBookMark')) {
                push @$t, $node;
            } else {
                splice @$t, $i, 0, $node;
    ...
    for (my $i = 0; $i < @$t; $i++) {
        if (@$t[$i]->{rid} == $find) {
            ...
            foreach my $key (sort keys %params) {
                @$t[$i]->{$key} = $params{$key}
                  if ($params{$key} && $key ne 'action' && $key ne 'folder' && $key ne 'subtree' && $key ne 'class' && $key ne 'dump');
                @$t[$i]->{$key} = ($m_hrSettings->{cgi}{mod_rewrite} ? "/$1.html" : "$ENV{SCRIPT_NAME}?action=$1")
                  if ($key eq 'href' && $params{$key} =~ /^action:\/\/(.*)$/);
            }
            saveTree($m_sDump, \@m_aTree);
            return;
