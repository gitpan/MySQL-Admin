my @menu = $m_oDatabase->fetch_AoH("select title,action,`submenu`,target from navigation where `right` <= $m_nRight order by position");
my @t    = fetchMenu(@menu);
...
sub fetchMenu
{
    ...
    for (my $i = 0; $i <= $#actions; $i++) {
        ...
        unless ($actions[$i]->{target}) {
            $fm = ($m_hrSettings->{cgi}{mod_rewrite}) ? "/$actions[$i]->{action}.html" : "$ENV{SCRIPT_NAME}?action=$actions[$i]->{action}";
        } else {
            $fm = $actions[$i]->{action};
        }
        if ($actions[$i]->{submenu}) {
            my @sumenu   = fetchMenu($m_oDatabase->fetch_AoH("select * from $actions[$i]->{submenu} where `right` <= $m_nRight order by title"));
