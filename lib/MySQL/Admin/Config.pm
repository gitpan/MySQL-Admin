...
@ISA                           = qw(Exporter);
$MySQL::Admin::Config::VERSION = '0.44';
...
sub saveConfig {
    ...
    if ($rsas) {
        open $fh, ">$rsas.bak"
            or warn
            "$/MySQL::Admin::Config::saveConfig$/ $! $/ File: $rsas $/Caller: "
            . caller()
            . $/;
        flock $fh, 2;
        seek $fh, 0, 0;
...
sub getSelf {
    return @_
        if defined( $_[0] )
            && ( !ref( $_[0] ) )
            && ( $_[0] eq 'MySQL::Admin::Config' );
    return ( defined( $_[0] )
                 && ( ref( $_[0] ) eq 'MySQL::Admin::Config'
                      || UNIVERSAL::isa( $_[0], 'MySQL::Admin::Config' ) )
    ) ? @_ : ( $MySQL::Admin::Config::DefaultClass->new, @_ );
}

=head2 see Also

L<CGI> L<MySQL::Admin> L<MySQL::Admin::Actions> L<MySQL::Admin::Translate> L<MySQL::Admin::Settings> L<MySQL::Admin::Config>
