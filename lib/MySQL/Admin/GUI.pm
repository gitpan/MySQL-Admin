...
<td>$wlc,</td><td valign="top">$m_sUser</td><td><a class="menuLink" href="$lg">logout</a></td></tr></table>
    ...
    my $query = param('query') ? param('query') : '';
    my %set = (
               zoom      => $zoom,
    ...
    if ($m_hrSettings->{sidebar}{right}) {
        ...
        if (defined $m_hrAct->{box}) {
            my @boxes = split /;/, $m_hrAct->{box};
            foreach my $box (@boxes) {
                my $bx = $m_oDatabase->fetch_hashref("select * from box where `dynamic` = 'right' && `file` = ? && `right` <= ? ", "$box.pl", $m_nRight);
                do("$m_hrSettings->{cgi}{bin}/Sidebar/$bx->{file}") if (defined $bx->{file} && -e "$m_hrSettings->{cgi}{bin}/Sidebar/$bx->{file}");
                warn "Error : $@ " if ($@);
            }
        }
