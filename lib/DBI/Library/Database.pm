...
$DBI::Library::Database::VERSION = '0.52';
$mod_rewrite                     = 0;
$min_secs                        = 10;
...
sub readMenu {
    ...
    while ( my @data = $sth->fetchrow_array() ) {
        ...
        my $nl =
            ($rewrite)
            ? "/$p_nStart/$p_nEnd/$thread.html#$id"
            : "$ENV{SCRIPT_NAME}?action=$thread&amp;von=$p_nStart&amp;bis=$p_nEnd#$id";
        push @output,
            { text => $headline,
              href => $nl,
            };
...
sub catright {
    ...
    for ( my $i = 0; $i <= $#cats; $i++ ) {
        $nRight = $cats[$i]->{right}
            if ( $sel{ $cats[$i]->{name} } && $cats[$i]->{right} > $nRight );
    }
    return $nRight;
}
...
sub addUser {
    ...
    my $mail        = $p[2];
    my $sql_addUser =
        qq/insert into users (user,pass,email,`right`,cats) values(?,?,?,1,'news|member')/;
...
sub searchDB {
    ...
    for ( my $i = 0; $i <= $#messages; $i++ ) {
        ...
        $messages[$i]->{title} =~
            s?($p[0])?<span style="color:red;">$1</span>?ig;
...
sub GetDataBases {
    ...
    my $return =
        qq|<select align="center" $change name="$name" style="width:75%" >|;
    $return .=
        $_ eq $m_sCurrentDb
