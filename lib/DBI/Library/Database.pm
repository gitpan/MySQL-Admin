...
%DBI::Library::Database::EXPORT_TAGS = (
    ...
    'dynamic' => [qw(getActionRight CurrentPass CurrentUser CurrentHost CurrentDb Driver useexecute void fetch_hashref fetch_AoH fetch_array updateModules deleteexecute editexecute addexecute)],
    'independent' =>
...
$DBI::Library::Database::VERSION = '0.62';
$mod_rewrite                     = 0;
$m_nSecs                         = 10;
...
sub searchDB
{
    ...
    $limit    = $limit    =~ /(\d+)/ ? $1 : 10;
    my $sql_select = "SELECT * FROM $p_sTable WHERE  `right` <= $p_nRight  && ( $p_sCol REGEXP $p_sQuery || title REGEXP $p_sQuery )  order by date desc LIMIT $p_nStart , $limit ";
    my $b          = '<table align="center" summary="layoutSearch" border="0" cellpadding="0" cellspacing="0" width="100%"><tr><td class="caption">Title</td><td class="caption">User</td><td class="caption">Datum</td></tr>';
...
sub fulltext
{
    ...
    $p_nStart = $p_nStart =~ /(\d+)/ ? $1 : 0;
    $limit    = $limit    =~ /(\d+)/ ? $1 : 10;
    my $b        = '<table align="center" summary="fulltext" border="0" cellpadding="2" cellspacing="0" width="100%"><tr><td class="caption">Title</td><td class="caption">User</td><td class="caption">Datum</td></tr>';
    my @messages = $self->fetch_AoH("SELECT * FROM $p_sTable  where `right` <= $p_nRight and MATCH (title,body) AGAINST($p_sQuery) order by date desc LIMIT $p_nStart , $limit");
