...
sub show_form
{
    ...
    foreach my $fieldref (@{$form{-FIELDS}}) {
        ...
        $form{-FIELDS}[$i]{-STYLE_FIELDVALUE} .= qq{ colspan="$form{-FIELDS}[$i]{-COLSPAN}"} if $form{-FIELDS}[$i]{-COLSPAN} and $form{-MULTI_COLUMN};    #"
...
sub _check_form
{
    ...
    foreach my $fieldref (@{$formref->{-FIELDS}}) {
        ...
        my ($valid, $why) =
          ...
          : (1, '');
        $formref->{-FIELDS}[$i]{-INVALID} = 1, $formref->{-FIELDS}[$i]{-WHY} = $valid ? undef : "<span$formref->{-STYLE_WHY}>$why</span>", $formref->{-INVALID}++
...
sub _on_valid_form
{
    ...
    return header, start_html('Quick Form'), h3('Quick Form'), p("You must define your own &amp;on_valid_form subroutine, otherwise " . "the data will simply be thrown away."), end_html,;
